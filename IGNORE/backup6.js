...
function wrapText(text, width) {
    text.each(function() {
        let textElement = d3.select(this),
            ...
            lineHeight = fontSize * 1.2, // Adjusts the line height based on font size.
        ...
        while (word = words.pop()) {
            ...
            if (tspan.node().getComputedTextLength() > width) {
                ...
                tspan = textElement.append("tspan")
                    .attr("x", 10)
                    .attr("y", y)
                    .attr("dy", (lineNumber + 1) * lineHeight + "px")
                    .text(word);
                lineNumber++;
            }
        }
...
function drawScene1(data) {
    ...
    let listSvg = d3.select("#jobTitleList");
    let textElement = listSvg.select("text")
        .attr("x", 10)
        .attr("y", 30)
        .attr("font-size", "16px")
        .attr("id", "defaultListText")
        .text("Click on a bar to see the distribution of the most common jobs for that salary range!");

    wrapText(textElement, 380); // Assuming a maximum width of 380 for the text given SVG width is 400.
...
function drawScene3(data, experienceLevel = "EN") {
    ...
    let textElement = listSvg.select("text")
        ...
        .attr("font-size", "16px")
        .attr("id", "defaultListText")
        .text("Use the dropdown to see salary trends for AI/ML Professionals of different levels of experience!");
